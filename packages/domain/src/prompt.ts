import type { PromptParts, PromptSlotKey } from './types.js'
...
export const REQUIRED_SLOTS: PromptSlotKey[] = [
  'character',
  'action',
  'environment',
  'mood',
...
/**
 * Assemble kid-friendly prompt from chip slots (scaffolded create, not open chat).
 */
...
export function missingSlots(parts: PromptParts): PromptSlotKey[] {
  return REQUIRED_SLOTS.filter((key) => !parts[key])
}
...
export function isPromptComplete(parts: PromptParts): boolean {
  return missingSlots(parts).length === 0
}
