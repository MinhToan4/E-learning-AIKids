...
def main():
    parser = argparse.ArgumentParser(description="Chạy tự động batch ảnh cho đối tượng thực hành qua FlowKit")
    parser.add_argument("--subject", default="goldfish", choices=["cat", "goldfish", "dog", "ceramic-cup", "notebook", "bicycle", "clock"], help="Đối tượng thực hành (mặc định: goldfish)")
    parser.add_argument("--project-id", help="UUID của Project trên flow.google.com", default=os.getenv("FLOW_PROJECT_ID"))
    parser.add_argument("--model", help="Flow Image Model", default="NARWHAL")
    parser.add_argument("--ref-media-id", help="Media ID của ảnh reference trên Flow")
    parser.add_argument("--ref-image", help="Đường dẫn file ảnh reference trên máy")
    ...
    parser.add_argument("--level", help="Lọc theo level (vd: goldfish_level1, goldfish_level2,...)")
    parser.add_argument("--overwrite", action="store_true", help="Ghi đè lại ảnh cũ kể cả khi file đã tồn tại")
    parser.add_argument("--delay", type=float, default=5.0, help="Số giây nghỉ giữa mỗi request (mặc định 5s để tránh rate limit)")

    args = parser.parse_args()

    subject = args.subject
    batch_file = ROOT_DIR / "data-export" / "flowkit" / "batches" / f"batch_{subject}.json"
    output_dir = ROOT_DIR / "apps/web/public/assets/pregenerated-combos" / subject
    ...
    print("=" * 65)
    print(f"🎨 FLOWKIT BATCH RUNNER - ĐỐI TƯỢNG: {subject.upper()}")
    print("=" * 65)
    ...
    if args.use_ref and not ref_media_id and args.ref_image:
        ref_path = Path(args.ref_image)
        if ref_path.exists():
            print(f"📤 Đang tự động upload ảnh reference: {ref_path.name}...")
            ref_media_id = upload_reference_image(ref_path, project_id)
