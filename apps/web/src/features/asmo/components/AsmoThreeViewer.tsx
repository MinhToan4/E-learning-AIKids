...
export function AsmoThreeViewer({
  ...
  useEffect(() => {
    ...
    // 3. Lights - Bright Soft Clay 3D Studio Setup
    ...
    const dirLight2 = new THREE.DirectionalLight(0xfef08a, 0.45)
  ...
  return (
    <div
      className={cn(
        'relative w-full rounded-3xl overflow-hidden bg-gradient-to-b from-sky-50/80 via-white to-brand-50/60 border-2 border-brand-100 shadow-clay',
        className,
      )}
    >
      {/* 3D Viewport */}
      ...
      {spec.template === 'GRID_PATH_MAZE' && onPathChange && (
        <div className="absolute top-3 right-3 flex items-center gap-1.5 rounded-2xl bg-white/90 px-3 py-1.5 backdrop-blur-md border-2 border-cyan-200 text-slate-800 shadow-clay">
          ...
          <div className="flex items-center gap-1 ml-1 border-l border-slate-200 pl-1.5">
            <button
              ...
              className="p-1 rounded-lg hover:bg-slate-100 active:scale-95 transition-all text-slate-600 cursor-pointer"
              title="Đường trước"
            >
            ...
            <button
              ...
              className={cn(
                'p-1 rounded-lg active:scale-95 transition-all cursor-pointer',
                isAutoPlayingPaths ? 'bg-cyan-500 text-white shadow-xs' : 'hover:bg-slate-100 text-slate-600',
              )}
      ...
      {interactive && (
        <div className="absolute bottom-3 left-3 right-3 flex items-center justify-between pointer-events-none">
            ...
            {spec.template === '3D_CUBE_CLUSTER' && (
              <>
                <button
                  ...
                  className="inline-flex items-center gap-1 rounded-2xl bg-white hover:bg-slate-50 px-3 py-1.5 text-xs font-bold text-slate-800 border-2 border-slate-200 shadow-clay backdrop-blur-md transition-all active:scale-95 cursor-pointer"
                >
                ...
                <button
                  ...
                  className={cn(
                    'inline-flex items-center gap-1 rounded-2xl px-3 py-1.5 text-xs font-bold border-2 shadow-clay backdrop-blur-md transition-all active:scale-95 cursor-pointer',
                    isWireframe
          ...
          <button
            type="button"
            onClick={handleReset}
            className="inline-flex items-center gap-1 rounded-2xl bg-white hover:bg-slate-50 px-3 py-1.5 text-xs font-bold text-slate-800 border-2 border-slate-200 shadow-clay backdrop-blur-md pointer-events-auto transition-all active:scale-95 cursor-pointer"
          >
      ...
      <div className="absolute top-3 left-3 pointer-events-none flex items-center gap-1.5 rounded-full bg-white/90 px-3 py-1 text-[11px] font-bold text-slate-800 backdrop-blur-md border border-brand-100 shadow-xs">
  ...
  switch (spec.template) {
    case '3D_CUBE_CLUSTER': {
      ...
      cubes.forEach(([x, y, z]) => {
        ...
        if (activeLayer >= 0) {
          if (y === activeLayer) {
            color = activeLayer === 0 ? 0xf59e0b : activeLayer === 1 ? 0x10b981 : activeLayer === 2 ? 0xf43f5e : 0x6366f1
            opacity = 1.0
          } else {
            opacity = 0.3
          }
    ...
    case 'INTERACTIVE_CLOCK': {
      ...
      // 12 Major & Minor Tick Marks (High contrast dark slate / highlighted colors)
      for (let i = 1; i <= 12; i++) {
        const isMajor = i % 3 === 0
        ...
        const markMat = new THREE.MeshStandardMaterial({
          color: isTargetHour ? 0xef4444 : isTargetMinute ? 0x0284c7 : isMajor ? 0x0f172a : 0x64748b,
      ...
      // Hour Hand (Ruby Red): Angle clockwise from 12
      ...
      // Minute Hand (Sky Cyan): Angle clockwise from 12
      ...
      // Center Pin (Polished Brass Gold)
    ...
    case 'SHADED_AREA_FRACTION': {
      ...
      // Outer pizza bright light wood plate
      ...
      for (let i = 0; i < total; i++) {
        ...
        let opacity = 1.0
        let color = 0xfbbf24 // Vibrant cheese yellow
        ...
        if (activeStep === 0) {
          ...
          // Step 2: 3 Eaten slices are lifted and highlighted pepperoni red
          if (isEatenSlice) {
            color = 0xf43f5e
            emissive = 0xe11d48
    ...
    case 'MATCHSTICK_FIGURE': {
      ...
      matches.forEach((m, matchIdx) => {
        ...
        const mat = new THREE.MeshStandardMaterial({
          color: isWingActive ? 0x0284c7 : 0xcbd5e1,
          emissive: isWingActive && activeStep >= 0 && activeStep < 4 ? 0x0369a1 : 0x000000,
          emissiveIntensity: isWingActive && activeStep >= 0 && activeStep < 4 ? 0.7 : 0,
        ...
        // Head of matchstick (Red / Pink sulfur)
    ...
    case '3D_BALANCE_SCALE': {
      ...
      // Ivory Porcelain / Warm Wood Pillar
      ...
      // Left & Right Pans: Pearl White Porcelain with Warm Gold Accent
      ...
      leftPanRim.position.set(-2, 0.34, 0)
      group.add(leftPanRim)
      ...
      rightPanRim.position.set(2, 0.34, 0)
      group.add(rightPanRim)

      // Left Pan: Emerald Green Watermelons
      ...
      // Right Pan: Sweet Vibrant Oranges
