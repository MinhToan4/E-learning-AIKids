...
describe('explorer XP levels', () => {
  it('resolves current and next level from lifetime XP', () => {
    ...
    expect(explorerLevelForXp(980_100, 100).level).toBe(100)
    expect(nextExplorerLevel(980_100, 100).level).toBe(101)
  })
  ...
  it('calculates bounded progress inside the current level', () => {
    ...
    expect(explorerLevelProgress(980_100, 100)).toBe(0)
  })
})
